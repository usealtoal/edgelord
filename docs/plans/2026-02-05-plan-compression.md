...
**Goal:** Prepend a standardized historical-summary header to all pre‑2026‑02‑05 plan files in `docs/plans/`.
...
**Files:**
- Test: `docs/plans/` (file list)

**Step 1: List plan files before 2026-02-05**

Run:
```bash
ls docs/plans | grep '^2026-02-0[1-4].*\\.md$' | sort
```
...
Run:
```bash
ls docs/plans | grep '^2026-02-0[1-4].*\\.md$' | wc -l
```
...
**Files:**
- Modify: `docs/plans/*.md` (pre‑2026‑02‑05 only)
...
Run:
```bash
sed -n '1,12p' docs/plans/2026-02-01-phase-1-foundation.md
sed -n '1,12p' docs/plans/2026-02-04-cli-stats-improvements.md
```
...
```bash
git add docs/plans/2026-02-0[1-4]*.md
...
Run:
```bash
grep -L \"^> Status: Historical\" docs/plans/2026-02-0[1-4]*.md
```
Expected: no output.
