...
use crate::application::position::{CloseReason, PositionManager};
use crate::application::risk::RiskManager;
use crate::port::RiskCheckResult;
use crate::application::statistic::{RecordedOpportunity, StatsRecorder};
use crate::adapter::strategy::{ConcreteDetectionContext, StrategyRegistry};
use crate::domain::{MarketRegistry, Opportunity};
use crate::error::RiskError;
use crate::port::{ArbitrageExecutor, Event, MarketEvent, OpportunityEvent, RiskEvent};
...
mod tests {
    ...
    use crate::infrastructure::cache::BookCache;
    use crate::infrastructure::{AppState, RiskLimits};
    ...
    fn handle_market_event_updates_cache_on_snapshot() {
        ...
        let db_pool = crate::adapter::store::db::create_pool("sqlite://:memory:").unwrap();
        let stats = statistic::create_recorder(db_pool);
        let position_manager = Arc::new(crate::application::position::PositionManager::new(
            Arc::clone(&stats),
        ));
