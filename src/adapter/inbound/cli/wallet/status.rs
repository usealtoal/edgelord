...
pub async fn execute_status(config_path: &Path) -> Result<()> {
    ...
    let pb = output::spinner("Fetching wallet status...");
    let status = match service.wallet_status(&config_toml).await {
        Ok(status) => {
            output::spinner_success(&pb, "Fetched wallet status");
            status
        }
        Err(e) => {
            output::spinner_fail(&pb, "Failed to fetch wallet status");
            return Err(e);
        }
    };
    ...
    if status.needs_approval {
        ...
        output::hint("run `edgelord wallet approve` to approve token spending");
    } else {
